...
SIM="$HERE/simulation/test_FUENTE3.sch"
DAT="$HERE/datos_fuente_v3"
STEP=${1:-5}
mkdir -p "$SIM" "$DAT"
...
echo "==> netlist"
rm -f "$SIM/test_FUENTE3.spice"
( cd "$HERE" && xschem -n -s -q -o "$SIM" test_FUENTE3.sch ) >/dev/null 2>&1 || true
test -s "$SIM/test_FUENTE3.spice" || { echo "  xschem did not regenerate the netlist" >&2; exit 1; }
for e in "xnav2 VDDV GND XPv Xv XNv YPv Yv YNv ZPv Zv ZNv S2P S2N S4P S4N S3P S3N S1P S1N GRADIENT_NAV2_V3" \
    ...
    inst=${e%% *}
    real=$(grep -m1 "^$inst " "$SIM/test_FUENTE3.spice" || true)
    if [ "$real" != "$e" ]; then
...
rows, ctrl = [], [".control"]
for dip in (0, 1):
    for rad in (2000, 3000, 4500, 6000, 9000, 12000, 18000):
        for tilt in range(0, 181, 60):
            i = len(rows) + 1
...
src = (SIM / "test_FUENTE3.spice").read_text().splitlines()
