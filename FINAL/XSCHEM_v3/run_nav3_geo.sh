...
SIM="$HERE/simulation/test_NAV3_geo.sch"
DAT="$HERE/datos_geo_v3"
...
echo "==> netlist"
rm -f "$SIM/test_NAV3_geo.spice"
( cd "$HERE" && xschem -n -s -q -o "$SIM" test_NAV3_geo.sch ) || true
if [ ! -s "$SIM/test_NAV3_geo.spice" ] || [ "$HERE/test_NAV3_geo.sch" -nt "$SIM/test_NAV3_geo.spice" ]; then
    echo "  xschem no regenero $SIM/test_NAV3_geo.spice" >&2; exit 1
fi
...
for e in "xnav2 VDDV GND XPv Xv XNv YPv Yv YNv ZPv Zv ZNv S2P S2N S4P S4N S3P S3N S1P S1N GRADIENT_NAV2_V3" \
         "xnavt VDDT GND XPt Xt XNt YPt Yt YNt ZPt Zt ZNt S2P S2N S4P S4N S3P S3N S1P S1N GRADIENT_NAV2"; do
    inst=${e%% *}
    real=$(grep -m1 "^$inst " "$SIM/test_NAV3_geo.spice" || true)
    if [ "$real" != "$e" ]; then
        echo "  CABLEADO ROTO en test_NAV3_geo.sch" >&2
...
def close_deck():
    ...
    src = (SIM / "test_NAV3_geo.spice").read_text().splitlines()
