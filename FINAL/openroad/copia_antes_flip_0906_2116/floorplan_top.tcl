...
foreach inst [$block getInsts] {
    ...
    set n 0
    foreach off $seen {
        #  `off` esta en coordenadas de core; los straps ocupan [off, off+need].
        set sx [expr {$off + $core_x0}]
        foreach band $libre {
            lassign $band lo hi
            if {$sx >= $lo && $sx + $need <= $hi} { incr n ; break }
        }
    }
...
puts [format "  Metal4 vertical: %d parejas x %.1f um = %.1f um -> %.1f mA (pide %.1f)  %s" \
          [llength $seen] $STRIPE_W [expr {[llength $seen] * $STRIPE_W}] \
          [expr {[llength $seen] * $STRIPE_W * 0.67}] $I_OBJ \
          [expr {[llength $seen] * $STRIPE_W * 0.67 >= $I_OBJ ? "OK" : "CORTO"}]]
...
proc extend_all_to_edge {block pin_name layer} {
    ...
    foreach sw [$net getSWires] {
        foreach box [$sw getWires] {
            ...
            lappend cajas [list 0 [$box yMin] [$box xMax] [$box yMax]]
        }
    }
    return $cajas
}
